...
export class Neu3D {
    ...
    constructor(container, data, metadata, options = {}) {
        ...
        this.settings.bloomPass = new PropertyManager({
            enabled: false,
            radius: 0.2,
            strength: 0.2,
            threshold: 0.3
        });
        this.settings.effectFXAA = new PropertyManager({
            enabled: false
        });
        this.settings.backrenderSSAO = new PropertyManager({
            enabled: false
        }); // change
        ...
        this.callbackRegistry = {
            'add': ((func) => {
                this.meshDict.on('add', func);
            }),
            'remove': ((func) => {
                this.meshDict.on('remove', func);
            }),
            'pinned': ((func) => {
                this.meshDict.on('change', func, 'pinned');
            }),
            'visibility': ((func) => {
            ...
            'click': ((func) => {
                this.uiVars.on('change', func, 'selected');
            }),
        };
        ...
        this.on('visibility', ((e) => {
            this.onUpdateVisibility(e.path[0]);
        }));
    ...
    addJson(json) {
        return new Promise((resolve) => {
            ...
            if (metadata.colororder === "order") {
                ...
                id2float = function(i) {
                    return getRandomIntInclusive(1, colorNum) / colorNum;
                };
            }
    ...
    getIntersection(groups) {
        ...
        for (const group of groups) {
            ...
            for (const ob of intersects) {
                ...
                if (Object.prototype.hasOwnProperty.call(object, 'rid') && object.rid in this.meshDict && object.visible) {
