...
Neu3D.prototype.loadSWCCallBack = function (key, unit, visibility) {
  return (swc_or_string, transformation=undefined) => {
    let swc = new SWC(swc_or_string, transformation);
    let object = swc.createObject(unit['color'], this.settings);
    object.visible = visibility;
    this._registerObject(key, unit, object);
  };
}
